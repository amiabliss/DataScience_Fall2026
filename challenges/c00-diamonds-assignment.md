...
## Your code here!
ggplot(data=diamonds) + geom_point(mapping=aes(x=price,y=carat,colour="pink"))
```
...
- general upward trend, with higher density along certain carats,
  seemingly in intervals of 0.5 (line visible at 1, 1.5, 2)
- No high density at 2.5 - something to note with dataset or is this a
  popularity/trend thing?
...
## Your code here!
ggplot(data=diamonds) + geom_smooth(mapping=aes(x=carat,y=price,colour=cut))
```
...
- We see that (excluding fair), the cut doesn’t impact the price as much
  as carat does. For the most part, the smoothed averages for the cuts
  are incredibly close together.
- At lower carats (\>1), a Fair graded cut follows that trend, but
  differs more when it comes to ~2 carats.
